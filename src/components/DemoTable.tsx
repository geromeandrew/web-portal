...
function columnWidth(header: string, index: number, widths: Record<number, number>) {
...
export default function DemoTable({ headers, rows, ariaLabel, caption, actions, actionWidth = 76, actionPosition = "start", columnWidths = {}, wrapColumns = [], emptyMessage = "No data available in table", pageSize = 25, groupBy }: DemoTableProps) {
  ...
  const tableWidth = (actions ? actionWidth : 0) + columns.reduce((total, column) => total + columnWidth(column.header, column.index, columnWidths), 0);
  ...
  const actionCell = (row: string[], rowIndex: number) => actions ? <td className={`sticky ${actionPosition === "start" ? "left-0 border-r" : "right-0 border-l"} z-10 border-slate-200 bg-white px-4 ${rowPadding} transition-colors group-hover:bg-[#F7FFFE]`}>{actions(row, rowIndex)}</td> : null;
  const renderRow = (row: string[], rowIndex: number) => <motion.tr key={`${row[0]}-${rowIndex}`} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2, delay: Math.min(rowIndex * 0.014, 0.14) }} className="group border-b border-slate-100 odd:bg-slate-50/[0.32] last:border-b-0 hover:bg-teal/[0.045]">{actionPosition === "start" ? actionCell(row, rowIndex) : null}{columns.map((column, displayIndex) => { const cell = row[column.index] ?? ""; const total = cell.startsWith("**"); const numeric = isNumeric(cell); const wrap = wrapColumns.includes(column.index); const displayValue = total ? cell.replaceAll("*", "") : cell || "-"; const stickyFirst = displayIndex === 0; return <td key={`${cell}-${column.index}`} style={stickyFirst ? { left: actionPosition === "start" && actions ? actionWidth : 0 } : undefined} title={cell || undefined} className={`border-r border-slate-100 px-4 ${rowPadding} transition-colors ${stickyFirst ? "sticky z-10 !border-slate-200 bg-white shadow-[8px_0_12px_-12px_rgba(15,23,42,.22)] group-hover:bg-[#F7FFFE]" : ""} ${numeric ? "text-right tabular-nums text-slate-600" : "text-slate-700"} ${total ? "font-bold text-slate-900" : ""}`}><span className={`block ${wrap ? "break-words whitespace-normal" : "truncate"} ${!cell ? "text-slate-200" : ""}`}>{total ? <span className="rounded-md bg-teal/8 px-1.5 py-0.5">{displayValue}</span> : displayValue}</span></td>; })}{actionPosition === "end" ? actionCell(row, rowIndex) : null}</motion.tr>;
