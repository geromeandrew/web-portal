  ...
  return <div className="min-h-screen bg-[#F7FBFC] text-slate-800">
    <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/85 backdrop-blur-xl">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:px-7">
        ...
        <div className="flex items-center gap-3"><span className="hidden text-xs text-slate-500 xl:inline">{user?.email}</span>{user?.isBootstrapAdmin ? <NavLink to="/admin/users" className="hidden text-xs font-semibold text-teal lg:inline">Users</NavLink> : null}<button onClick={() => void logout()} className="focus-ring grid h-9 w-9 place-items-center rounded-lg border border-slate-200 text-slate-600" aria-label="Sign out"><LogOut className="h-4 w-4" /></button><button onClick={() => setMenuOpen((value) => !value)} className="focus-ring grid h-9 w-9 place-items-center rounded-lg border border-slate-200 text-slate-600 lg:hidden" aria-label="Toggle navigation">{menuOpen ? <X className="h-4 w-4" /> : <Menu className="h-4 w-4" />}</button></div>
      </div>
