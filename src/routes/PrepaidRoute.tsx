...
type Slot = { expected: string; status: SlotStatus; uploadId?: string; name?: string; size?: string; created?: string; progress?: number; message?: string };
...
export default function PrepaidRoute() {
  ...
  const [workflowState, setWorkflowState] = useState<{ egLayout: { frozen: boolean; rows: string[][] }; sgLayout: { frozen: boolean; rows: string[][] }; allocation: { rows: string[][] }; jv: { rows: string[][] }; sourceFiles: { expected: string[] } } | null>(null);

  const loadState = async () => {
    const state = await apiRequest<{ egLayout: { frozen: boolean; rows: string[][] }; sgLayout: { frozen: boolean; rows: string[][] }; allocation: { rows: string[][] }; jv: { rows: string[][] }; sourceFiles: { expected: string[] }; uploads: Array<{ id: string; slot?: string; originalName: string; size: number; uploadedAt: string }> }>("/workflows/prepaid/state");
    setWorkflowState(state);
    setFrozen({ "eg-layout": state.egLayout.frozen, "sg-layout": state.sgLayout.frozen });
    setSlots(state.sourceFiles.expected.map((expected) => {
      const record = state.uploads.find((upload) => upload.slot === expected);
      return record ? { expected, status: "success", uploadId: record.id, name: record.originalName, size: `${(record.size / 1_000_000).toFixed(2)}`, created: new Date(record.uploadedAt).toLocaleString() } : { expected, status: "empty" };
    }));
  };

  useEffect(() => { void loadState().catch((error: Error) => setNotice(error.message)); }, []);
  ...
  const selectSlotFile = async (expected: string, file?: File) => {
    ...
    try {
      await uploadSingleFile(file, (progress) => setSlots((current) => current.map((slot) => slot.expected === expected ? { ...slot, progress } : slot)), "prepaid", expected);
  ...
  const currentRows = section === "eg-layout" ? workflowState?.egLayout.rows ?? [] : workflowState?.sgLayout.rows ?? [];
