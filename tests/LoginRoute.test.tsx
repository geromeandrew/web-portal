...
describe("LoginRoute", () => {
  ...
  it("renders the ES-ATP sign-in experience", async () => {
    ...
    expect(container.textContent).toContain("Automation and Transformation Platform");
    expect(container.querySelector('img[alt="Person using a laptop"]')).not.toBeNull();
    expect(container.querySelector("button")?.textContent).toContain("Log in with OKTA SSO");
  });

  it("starts Okta sign-in with the requested return path", async () => {
    await renderLogin({ pathname: "/login", state: { from: "/processing-pipelines" } } as never);
  ...
  it("restores the sign-in button and announces an error when sign-in cannot start", async () => {
    ...
    expect(container.querySelector('[role="alert"]')?.textContent).toBe("Unable to start Okta sign-in. Please try again.");
